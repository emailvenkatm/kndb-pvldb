#!/usr/bin/env bash
# G2.3 (v2) — Adversarial + throughput on Synthea-preloaded kndb.fact (565k rows),
# under the v2 engine (precedence lattice + use-permission views + specificity).
#
# Assumes:
#   - Native ProvSQL install healthy at postgresql://kndb_native:kndb_native@localhost:5434/kndb_native
#   - v2 engine already applied by run_native_v2.sh (fresh schema, uppercase enum).
...
#   3. Results land in bench/results/native_synthea_v2/ so v1 numbers at
#      bench/results/native_synthea/ stay untouched for the comparison table.

set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
RESULTS_DIR="${REPO}/bench/results/native_synthea_v2"
mkdir -p "$RESULTS_DIR"

PSQL="/opt/homebrew/opt/postgresql@17/bin/psql -h localhost -p 5434 -U kndb_native -d kndb_native -X -v ON_ERROR_STOP=1"

log() { printf "[run_synthea_v2] %s\n" "$*"; }
...
cat > "$RESULTS_DIR/manifest.json" <<EOF
{
  ...
  "preserve_facts":        true,
  "engine":                "v2",
  "kndb_fact_rows_before": $($PSQL -tAc "SELECT count(*) FROM kndb.fact WHERE upper(sys_time)='infinity'" | tr -d ' '),
  "note":                  "Native ARM64 + Synthea 565k-row preload for P4A v2. v2 engine (precedence lattice + use-permission views + specificity column) enabled."
}
EOF
