...
The \texttt{INSERT \dots ON CONFLICT} path (F21) is a defensive override: the SQL surface for the speculative-insertion attack is currently unreachable on epistemic tables because \texttt{CREATE UNIQUE INDEX}, \texttt{ADD PRIMARY KEY}, \texttt{ADD UNIQUE}, and \texttt{ADD EXCLUDE} all fail at \texttt{heap\_getnext}'s \texttt{rd\_tableam} identity check (\texttt{heapam.c:1352}) during the arbiter index's build scan, so \texttt{ON CONFLICT (col)} has no arbiter to bind (empirical probe: \texttt{scripts/speculative\_forgery.sh}). To ensure KNDB's enforcement survives future index-support expansions, the F21 callbacks explicitly intercept the speculative-insertion path. The \texttt{epistemic.\_probe\_speculative\_insert} C-level test and the disable-and-test in Section~\ref{sec:eval-f21} verify this isolation.

The engine-in-storage differentiator therefore holds against a writer with \texttt{INSERT}, \texttt{UPDATE}, \texttt{DELETE}, plus \texttt{ALTER TABLE} on the target relation, or a role that can toggle \texttt{session\_replication\_role} (a replication or CDC operator, a migration tool, a connection pool). It does not hold against the table owner executing DDL modifications (e.g., \texttt{ALTER TABLE \dots SET ACCESS METHOD heap}). We frame this explicitly as defense-in-depth: KNDB operates at the storage tier to enforce the epistemic contract against routine DML bypasses, while standard PostgreSQL Role-Based Access Control (RBAC) is expected to secure the administrative DDL perimeter. This isolates epistemic integrity from operational replication or migration pipelines, which is the exact intended boundary for a storage engine.
...
\textbf{Zero on temporal knowledge-editing benchmarks.} The \texttt{xmin} tiebreak enforces first-committer-wins to guarantee strict deterministic conflict resolution under high concurrency, explicitly trading away last-writer-wins semantics. Consequently, KNDB fails by design on temporal knowledge-editing benchmarks (e.g., MemoryAgentBench Conflict\_Resolution, LongMemEval, and MQuAKE-CF-3k) where truth is defined purely by chronological arrival rather than epistemic rank.

\textbf{Reputation farming defeats $k^{*}=\infty$.} The argument depends on adversarial identities appearing fresh in the write stream so engine-assigned kind places them in \texttt{INFERRED}. A determined adversary who knows the tier-mapping rule can farm reputation to reach Tier A before attacking: complete the Zheng qualification test (question ids 2000--2019) so \texttt{quali\_acc} matches Tier A, or publish benign \texttt{n\_listings} volume on Book-Author to enter the top-$K$. Once inside Tier A the writes are \texttt{MEASURED} and the lattice defends nothing. Defending against long-term reputation farming requires a dynamic, user-space policy engine (e.g., KYC or signed source attestations) to manage tier mobility. The database kernel's responsibility is to guarantee that \emph{if} independent metadata is supplied, an adversary cannot forge their way out of it at write time. Because KNDB evaluates precedence at the \texttt{heapam} boundary, managing external identity lifecycles remains safely out of scope as a policy-tier concern.

\textbf{Write-path performance costs (SPI and Seqscan).} As noted in Section~\ref{sec:eval-f9-gate}, single-thread throughput drops by 71.1\% at zero contention compared to raw heap. This is driven by two specific bottlenecks. First, both R2 (source resolution) and R5 (slot-kind check) run a full \texttt{SPI\_connect}/\texttt{execute\_with\_args}/\texttt{finish} cycle per candidate row. A 5\,000-row microbenchmark puts R2's overhead at $\sim$7\,$\mu$s per row. Future versions can optimise this by preparing and caching the SPI plan via \texttt{SPI\_saveplan} in backend-local memory to avoid repeated parsing on high-frequency write paths. Second, because the \texttt{rd\_tableam} identity check blocks a standard arbiter index, \texttt{find\_live\_overlap} is forced to perform an unindexed sequential scan. Future work could maintain a lightweight, concurrent hash table in shared memory or a custom Bloom filter to bypass the seqscan for empty slots, mitigating the worst-case throughput penalty while preserving correctness.

\textbf{Batch-size ceiling of $\sim$15\,000 rows per transaction at default GUC.} The reliance on transaction-scope advisory locks ensures correct concurrency control but saturates PostgreSQL's default fast-path lock table (\texttt{max\_locks\_per\_transaction=64}) at approximately 15\,000 distinct-slot writes per transaction. While operators can tune this GUC, achieving higher native throughput without exhaustion would require pushing conflict arbitration further down into a custom page-level latching mechanism, bypassing the shared lock manager.
